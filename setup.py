...
# parse data
cluster_colors_path = Path("data/metadata/WMB-taxonomy/20231215/views/cluster_to_cluster_annotation_membership_color.csv")
cluster_details_path = Path("data/metadata/WMB-taxonomy/20231215/views/cluster_to_cluster_annotation_membership_pivoted.csv")
...
for d in datasets:
    ...
    if (all(p.exists() for p in [cells_path, coexp_path, presence_path, obs_path, var_path]) and not args.overwrite):
        continue
    ...
    abc_cache.get_file_path(directory=f"{d}", file_name=f"{d}/raw")
    ...
    df = (metadata.merge(cluster_details, on="cluster_alias")
